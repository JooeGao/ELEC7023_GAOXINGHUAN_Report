...
import torch.nn.functional as F  # 导入功能模块
...
input = torch.ones((1, 3, 28, 28)).to(device)  # 创建一个28x28的输入数据
print('original input size: 28x28')

# 调整输入图像大小为224x224
input = F.interpolate(input, size=(224, 224), mode='bilinear', align_corners=False)
print('resized input size: 224x224')
